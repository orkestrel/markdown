...
| Concern              | `sanitizeUrl` (here)                                                                                                                                           | `@orkestrel/html`'s `sanitizeURL`                                                                                                            |
| -------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| Allowlist            | `SAFE_URL_SCHEMES` is fixed and closed, so `javascript:` / `data:` / `vbscript:` / `file:` are refused BY the allowlist — no destination argument can widen it | the caller's allowlist REPLACES the default, so it adds an unwidenable refusal of those same four schemes that no option can argue past      |
...
| Escaping             | inside `sanitizeUrl`, exactly once — its result is a finished `href` attribute value                                                                           | not here at all; the raw surviving value is encoded later, at serialization                                                                  |
