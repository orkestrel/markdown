...
| `LineBreakNode`             | interface | `{ element: 'break' }` — a GFM hard break, rendered as two spaces followed by a newline.                                                                                                                                       |
...
| `MarkdownCell`              | interface | `{ heading: boolean, align: TableAlign \| undefined, inlines: readonly InlineNode[] }` — one projected `th` / `td`; a row is simply its cells, and the header row is the first a `th` heads.                                   |
| `MarkdownProjection`        | interface | `{ blocks, inlines, text, cells, rows }` — what one HTML node projects to; the fold value `htmlToMarkdown` carries up the AST.                                                                                                 |
...
Pure, total parsing + writing leaves from [`helpers.ts`](../../src/core/helpers.ts) — the functional core `parsers.ts` composes and the projections `Markdown` and callers reach for directly (AGENTS §5). Every function is unit-testable in isolation; malformed input degrades to text, never throws.
...
| `markdownToHTML`      | function | `(node: MarkdownNode) => HTMLDocument`                                                               | Pure, unsanitized projection of any `MarkdownNode` into an `@orkestrel/html` document.                                                                                                                                                                                                                                                                                                                                                         |
| `renderHTML`          | function | `(node: MarkdownNode) => string`                                                                     | Composes `markdownToHTML` with `@orkestrel/html`'s sanitize floor and serializer, widening allowed attributes by exactly `src` for images.                                                                                                                                                                                                                                                                                                     |
...
| `htmlToMarkdown`      | function | `(node: HTMLNode) => MarkdownDocument`                                                               | Projects an `@orkestrel/html` node to a markdown document — the inverse of `markdownToHTML`. Re-sanitizes every `href` / `src`, drops every `UNSAFE_ELEMENTS` subtree whole, and emits canonical markdown shapes so the projection re-parses to itself.                                                                                                                                                                                        |
...
| `projectLeaf`         | function | `(leaf: CommentNode \| DoctypeNode \| TextNode) => MarkdownProjection`                               | Projects one HTML leaf: text with its whitespace runs collapsed (its raw value kept for code), a comment and a doctype to nothing.                                                                                                                                                                                                                                                                                                             |
| `projectNode`         | function | `(node: ElementNode \| HTMLDocument, children: readonly MarkdownProjection[]) => MarkdownProjection` | THE element mapping: which markdown node each HTML tag becomes, with every other element unwrapped to its children and every `UNSAFE_ELEMENTS` subtree dropped whole.                                                                                                                                                                                                                                                                          |
...
**Why this floor is re-implemented rather than shared.** `@orkestrel/html` enforces the same lower floor in its own `sanitizeURL` — strip every codepoint ≤ `U+0020` and `U+007F`–`U+009F`, refuse any two-character protocol-relative prefix drawn from `/` and `\`, extract an ASCII scheme, enforce an allowlist, keep relative / anchor / scheme-less values. That duplication is deliberate: a sanitizer belongs to the output context it defends, and each package escapes at a different point in a different pipeline. Above the shared floor the two diverge, in three ways that are properties of those contexts rather than drift:

| -------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| Character references | none needed — the surviving value is escaped here, so `&#106;avascript:x` reaches the browser as literal text whose `:` never begins a scheme | decoded to a bounded fixpoint BEFORE the scheme check, because its sanitized value is re-serialized (and can be reparsed) further downstream |

Because there is no shared function to test once, the adversarial corpus is what the two packages share: the same vectors — control and whitespace splices, case variance, all four protocol-relative forms, kept relative/anchor/query destinations, refused schemes, entity-obfuscated schemes — are mirrored vector-for-vector and group-for-group in both test suites ([`tests/src/core/helpers.test.ts`](../../tests/src/core/helpers.test.ts) here, and its counterpart there), so a vector missing from one list is missing from both and each divergence above is pinned by a named test rather than an absent assertion.
